...
y = [line.split(',')[d:(d+1)] for line in inputDATA]
for i in range(len(y)): # Not pythonic =(
...
for i in range(len(Classes)): # For each class:
    y_hat_class = train_y_hat==i
    ytrain_class = (ytrain==i).ravel()
    Positives = set(np.where(ytrain_class==1)[0]) # This is the real positives
    PPositives = set(np.where(y_hat_class==1)[0]) # This is the predicted positives
    total = len(Positives) # Amount of real positives
    predicts = len(Positives.intersection(PPositives)) # Amount of predicted positives
    print("Accuracy of Class", i, "was:", (predicts/total)*100, "%")
